...
void llama_model_qwen4exp::load_arch_hparams(llama_model_loader & ml) {
    // NextN/MTP: n_layer_nextn is already read generically (llama-model.cpp, before
    // load_arch_hparams runs), so n_layer() == n_layer_all - n_layer_nextn is valid here
    // without re-reading it — this array's per-layer count still spans every block
    // including the trailing MTP one.
...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    // draft-head-only files (no trunk tensors) ship without the hc head mixer;
    // the MTP graph falls back to nextn.shared_head_norm + the block's own ffn
    ...
    // flat [ple_head_dim, n_rows] gather target. Guarded by !mtp_only: a draft-head-only
    // file (no trunk) carries no PLE tensors at all, so this whole block is meaningless there.
